...
# This version of the script has been modified to be able to be run during the PSI tests.
#
# Author: Abtin Ghodoussi | abtin.ghodoussi@mda.space
#         Tara Kong       | tara.kong@mda.space
# Date: May 22 2024
...
display_help_banner()
{
    echo "\
    ****************************************************************************
    update_qspi_remote.sh ARCHIVE TALON_BOARD TALON_IP PARTITION [-no_reboot] [-f]
    ...
    TALON_BOARD:
        talon-001, talon-002, talon-003, etc...
        Used to print friendly name in script output
    TALON_IP:
        IP address of TALON_BOARD
        Used to determine any information for the relevant talon voard in place of ambiguous identifier.
    PARTITION:
...
# for example from "root@talon-001" extract "talon-001"
# or from "root@192.168.8.1" extract "192.168.8.1"
talon=$(echo "$talon_arg"| cut -d'@' -f 2)

if [[ -z $talon ]] || [[ -z $talon_ip ]] || [[ -z $ARCHIVE ]] || [[ -z $REQUESTED_QSPI_PARTITION ]]; then
    display_help_banner
    exit 1
fi
...
wait_for_valid_ping ()
{
    ...
    do
        ...
        PING_COMMAND=$(ping -w 5 -c 3 $TARGET_BOARD_IP 2>&1)
        ping_result=$?
...
is_partition_empty()
{
    ...
    is_disabled=$(ssh -vvv -o StrictHostKeyChecking=no root@"$target_board_ip" -n "rsu_client --list $requested_slot" | awk '/PRIORITY/ {print $2}')
...
compare_qspi_images ()
{
    ...
    if is_partition_empty "$target_board_ip" "$slot_num"; then
        return 1
    fi

    ssh -vvv -o StrictHostKeyChecking=no root@"$target_board_ip" -n "rsu_client --copy /lib/firmware/current_qspi_slot${slot_num}.bin --slot $slot_num"

    #count the number of differences between the requested file and the programmed file 
    num_diff=$(ssh -vvv -o StrictHostKeyChecking=no root@"$target_board_ip" -n "cmp -l /lib/firmware/$requested_qspi_file /lib/firmware/current_qspi_slot${slot_num}.bin | wc -l")
    #clean-up
    ssh -vvv -o StrictHostKeyChecking=no root@"$target_board_ip" -n "rm /lib/firmware/current_qspi_slot${slot_num}.bin"
...
remote_log_db(){
    key=$1
    value=$2
    ssh -vvv root@"$talon_ip" -n "/bin/qspi_partition.sh -dbw $key $value"
}
...
remote_log_partition_version()
{
    ...
    current_date=$(date +%Y-%m-%d_%H:%M-UTC)
...
if ! wait_for_valid_ping "$talon" "$talon_ip" 40; then
...
#copy the file over
scp -vvv -o StrictHostKeyChecking=no "$application_update" root@"$talon_ip":/lib/firmware

echo "Requesting log from rsu_client..."
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --log"; check_error $? "failed to request the log";
echo "Requesting number of slots from rsu_client..."
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --count"; check_error $? "failed to request the number of slots";

compare_qspi_images "$application_update_basename" "$talon_ip" "$TARGET_QSPI_SLOT"
result=$?

QSPI_VERSION=$(ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "/bin/qspi_partition.sh -i | grep 'partition{0}' | grep -oE '[0-9](\.[0-9]+)+|unofficial'")
echo "QSPI Version: $QSPI_VERSION"

if [ "$result" -eq 0 ] ; then
    #request the board to switch to the required partition if needed
    echo "Requesting a partition switch from the board..."
    ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "/bin/qspi_partition.sh $REQUESTED_QSPI_PARTITION $REBOOT_REQUEST"
    result=$?
    ...
    elif ! wait_for_valid_ping "$talon" "$talon_ip" 40; then
...
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --erase $TARGET_QSPI_SLOT"; check_error $? "failed to erase the slot";

#write and verify slot
echo "Writing the image..."
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --add /lib/firmware/$application_update_basename --slot $TARGET_QSPI_SLOT"; check_error $? "failed to write the image";
echo "Verifying the image..."
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --verify /lib/firmware/$application_update_basename --slot $TARGET_QSPI_SLOT"; check_error $? "failed to verify the image";

#request slot to be loaded explicitly
echo "requesting/enabling slot${TARGET_QSPI_SLOT}" 
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --request $TARGET_QSPI_SLOT"; check_error $? "failed to request the image explicitly";
ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "rsu_client --enable $TARGET_QSPI_SLOT"; check_error $? "failed to enable the image explicitly";
...
if ! $SKIP_REBOOT; then
    ...
    ssh -vvv -o StrictHostKeyChecking=no root@"$talon_ip" -n "reboot -n && exit"
